...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Gestione del caricamento dei nuovi video
    $upload_dir = "../uploads/doors/" . $program['door_id'] . "/ad_video/";
    ...
    foreach ($_FILES['videos']['tmp_name'] as $key => $tmp_name) {
        ...
        if (move_uploaded_file($tmp_name, $target_file)) {
            $videos[] = "/panel/uploads/doors/" . $program['door_id'] . "/ad_video/" . $file_name;
        }
    }

    $videos_json = json_encode($videos);

    $stmt = $conn->prepare("UPDATE schedule SET title = ?, description = ?, start_time = ?, end_time = ?, videos = ? WHERE id = ?");
    if (!$stmt) {
        die("Prepare failed: " . $conn->error);
    }
    $stmt->bind_param("sssssi", $title, $description, $start_time, $end_time, $videos_json, $program_id);
    if ($stmt->execute()) {
        header("Location: view_door.php?id=" . $program['door_id']);
        exit();
    } else {
...
<head>
  ...
  <title>Modifica Programma</title>
  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Modifica Programma</h1>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </div>
              ...
              <form action="edit_program.php?id=<?php echo $program_id; ?>" method="post" enctype="multipart/form-data">
                <div class="card-body">
                  ...
                  <div class="form-group">
                    ...
                    <textarea class="form-control" id="description" name="description"><?php echo htmlspecialchars($program['description']); ?></textarea>
                  </div>
                  <div class="form-group">
                    <label for="start_time">Ora di Inizio</label>
                    <input type="datetime-local" class="form-control" id="start_time" name="start_time" value="<?php echo date('Y-m-d\TH:i', strtotime($program['start_time'])); ?>" required>
                  </div>
                  <div class="form-group">
                    <label for="end_time">Ora di Fine</label>
                    <input type="datetime-local" class="form-control" id="end_time" name="end_time" value="<?php echo date('Y-m-d\TH:i', strtotime($program['end_time'])); ?>" required>
                  </div>
                  <div class="form-group">
                    <label for="videos">Video</label>
                    <input type="file" class="form-control" id="videos" name="videos[]" multiple>
                    <div id="video-thumbnails" class="mt-2">
                      <?php
                      $videos = json_decode($program['videos'], true);
                      if (!empty($videos)) {
                          foreach ($videos as $video) {
                              echo '<video width="100" controls><source src="' . htmlspecialchars($video) . '" type="video/mp4"></video>';
                          }
                      }
                      ?>
                    </div>
